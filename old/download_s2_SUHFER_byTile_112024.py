...
s2 = os.path.join(home, 'tifs_byTile')
    ...
    for m in months:
        for t in tiles:
            ...
            for d in range(0,len(date_list)-1):
                ...
                try:
                    r = requests.get(f"https://catalogue.dataspace.copernicus.eu/odata/v1/Products?$filter=startswith(Name, 'S2A_MSIL2A') and contains(Name, '{t}') and ContentDate/Start gt {start_date}.000Z and ContentDate/Start lt {end_date}.000Z").json()
